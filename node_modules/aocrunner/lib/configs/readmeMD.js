...
const renderDayBadges = (config) => {
  return config.days.map(({ part1, part2 }, index) => {
    const day = String(index + 1).padStart(2, "0");
    const color = part1.solved && part2.solved || part1.solved && day === "25" ? "green" : part1.solved || part2.solved ? "yellow" : "gray";
    const badge = `![Day](https://badgen.net/badge/${day}/%E2%98%8${part1.solved ? 5 : 6}%E2%98%8${part2.solved || part1.solved && day === "25" ? 5 : 6}/${color})`;
...
const renderResults = (config) => {
  let totalTime = 0;
  let totalStars = 0;
  const results = config.days.map(({ part1, part2 }, index) => {
    const day = String(index + 1).padStart(2, "0");
    let timeBoth = 0;
    if (part1.solved) {
      totalStars++;
...
var readmeMD_default = readmeMD;
export {
  readmeMD_default as default,
  renderDayBadges,
  renderResults
};
