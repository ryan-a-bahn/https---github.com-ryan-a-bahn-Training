...
const send = async (config, dayNum, part) => {
  console.log(`
Part ${part}:`);
  const dayData = part === 1 ? config.days[dayNum - 1].part1 : config.days[dayNum - 1].part2;
  if (dayData.solved) {
...
const dev = (dayRaw) => {
  ...
  chokidar.watch("src", { ignoreInitial: true }).on("add", reload).on("change", reload);
  const listenToInput = async () => {
    ...
    switch (command.toLowerCase()) {
      case "fetch":
      case "f":
        getInput(config2.year, dayNum, inputPath);
        break;
      case "send":
      case "s":
        const solved = await send(config2, dayNum, 1);
        if (solved) {
          await send(config2, dayNum, 2);
        }
        break;
      case "help":
      case "h":
